...
def build(events: pd.DataFrame, placement: str):
    """Interval counts, meter signal, background shape and truth for one placement."""
    ...
    return counts, target, shape, truth
...
def compare(events: pd.DataFrame, scope: str, placement: str) -> dict:
    ...
    X, y, shape, truth = build(events, placement)
    ...
    return {
        ...
        "intervals_with_energy_but_no_count": int(
            ((np.abs(y) > 1e-9) & (X.sum(axis=1) <= 1e-9)).sum()
        ),
    }
