"""
MarkovModel_v2.py — two-variant event-based energy model (BPI Challenge 2019).

Pipeline (run via main()):
  1. Load XES log
  2. Find the top 2 most frequent variants
  3. Build EventObjects (tagged with variant_id, simulated durations)
  4. Generate synthetic signal from both variants combined
...
# one event = one activity in one case, plus a random duration.
# now it also remembers which variant it belongs to.
@dataclass
class EventObject:
    e_id: int
    case_id: str
...
def find_top_k_variants(
    ...
    # for each of the top k: keep its id, its sequence, its case ids and case count
...
def create_event_objects_for_cases(
    ...
    # build one EventObject per row for these cases, tagging them with the variant id
...
def inject_event_costs(
    ...
    # for each 30-min slot, sum the energy of all events inside it (slow nested loop)
...
def generate_two_variants_signal(
    events: Sequence[EventObject],
    output_dir: Path,
    freq: str = "30min",
) -> Tuple[Path, pd.DataFrame]:
    # build the signal from both variants' events together and save it
    if not events:
    ...
    # the three parts: background wave, noise, event spikes
    ...
    # save clean (no noise) and noisy versions
    df = pd.DataFrame(
        {
            "timestamp": timeline,
            "baseline": baseline,
    ...
    out_path = output_dir / "two_variants_signal.csv"
    df.to_csv(out_path, index=False)
    return out_path, df


def build_two_variants_cost_table(
    events: Sequence[EventObject],
    event_cost_scale: float = 1.0,
    duration_scale: float = 0.01,
) -> pd.DataFrame:
    ...
    # most expensive rows first
    table = pd.DataFrame(rows).sort_values("estimated_total_cost", ascending=False).reset_index(drop=True)
    return table


def build_two_variants_event_matrix(
    events: Sequence[EventObject],
    signal_df: pd.DataFrame,
    freq: str = "30min",
) -> pd.DataFrame:
    # grid of event counts per 30-min slot, across both variants
    if not events:
...
def estimate_state_energy_iterative(
    ...
    # start every energy at 1, then refine one state at a time, 20 passes.
...
def main() -> None:
    # run the whole pipeline for the top 2 variants
    thesis_dir = Path(__file__).resolve().parent
    xes_path = thesis_dir / "BPI_Challenge_2019.xes"
    output_dir = thesis_dir / "generated_signals_v2"
    output_dir.mkdir(parents=True, exist_ok=True)

    # 1. load the log and pick the top 2 variants
    df = load_xes_dataframe(xes_path)
    selected_variants, variant_counts = find_top_k_variants(df, k=2)

    print("Two-variant mode: modeling top 2 most frequent variants.")
    ...
    for variant_id, _, case_ids, _ in selected_variants:
        variant_events = create_event_objects_for_cases(
            ...
            case_ids,
            variant_id=variant_id,
            random_seed=42 if variant_id == "variant_1" else 43,
        )
        events.extend(variant_events)

    print(
        f"\nBuilt EventObject list for both variants: "
    ...
    overview_path = output_dir / "two_variants_overview.csv"
    overview_df.to_csv(overview_path, index=False)
    print(f"\nSaved variant overview to: {overview_path}")

    # 4. build and save the signal
    written_file, signal_df = generate_two_variants_signal(events, output_dir=output_dir, freq="30min")
    print("\nGenerated synthetic signal for Variant 1 + Variant 2 events:")
    print(f"- {written_file}")

    # 5. cost table
    cost_table = build_two_variants_cost_table(events)
    cost_table_path = output_dir / "two_variants_cost_table.csv"
    cost_table.to_csv(cost_table_path, index=False)
    print("\nTwo-variant cost table (sample):")
    print(cost_table.head(12).to_string(index=False))
    print(f"\nSaved full two-variant cost table to: {cost_table_path}")

    # 6. event matrix
    event_matrix = build_two_variants_event_matrix(events, signal_df, freq="30min")
    event_matrix_path = output_dir / "two_variants_event_matrix_30min.csv"
    event_matrix.to_csv(event_matrix_path, index=False)
    print("\nSaved two-variant event matrix (30-minute intervals) to:")
    print(f"- {event_matrix_path}")

    # 7. pooled markov chain (both variants mixed) + its plot
    pooled_markov_df = build_markov_chain(events)
    pooled_markov_path = output_dir / "two_variants_markov_chain_pooled.csv"
    pooled_markov_df.to_csv(pooled_markov_path, index=False)
    print("\nSaved pooled Markov chain (both variants, data-driven probabilities) to:")
    print(f"- {pooled_markov_path}")

    # one markov chain + plot per variant (probabilities are all 1 inside a variant)
    ...
    # plot the pooled chain too (auto layout)
    pooled_plot_path = plot_markov_chain(
        pooled_markov_df,
        output_dir / "two_variants_markov_chain_pooled.png",
        title="Pooled two-variant Markov chain (transition probabilities on edges)",
        sequence_order=None,
    )
    print(f"Saved pooled Markov chain plot to: {pooled_plot_path}")

    # 8. recover the energies and compare to the true values
    ...
    learning_path = output_dir / "two_variants_markov_state_learning.csv"
    ...
    learning_summary_path = output_dir / "two_variants_markov_state_learning_summary.csv"
